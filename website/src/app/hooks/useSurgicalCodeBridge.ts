...
export function useSurgicalCodeBridge() {
    ...
    const initialResult = useMemo(() => {
        const code = CODE_TEMPLATE
            .replace("__PADDING__", `p-[24px]`)
            .replace("__BORDER_RADIUS__", `rounded-[12px]`)
            .replace("__OPACITY__", `opacity-[100%]`);
        return { code, changedLines: [], changedTokens: new Map() } as PatchResult;
    }, []);
